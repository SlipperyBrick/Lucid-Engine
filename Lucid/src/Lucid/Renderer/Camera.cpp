...
{
	// Max pan speed on x-axis is 2.4f
	float x = std::min(m_ViewportWidth / 1000.0f, 2.4f);
	float xFactor = 0.0366f * (x * x) - 0.1778f * x + 0.3021f;

	// Max pan speed on y-axis is 2.4f
...
void Camera::OnEvent(Event& e)
{
	...
	dispatcher.Dispatch<MouseScrolledEvent>(LD_BIND_EVENT_FN(Camera::OnMouseScroll));
}
